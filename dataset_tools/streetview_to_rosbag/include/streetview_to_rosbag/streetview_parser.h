#ifndef STREETVIEW_TO_ROSBAG_STREETVIEW_PARSER_H_
#define STREETVIEW_TO_ROSBAG_STREETVIEW_PARSER_H_
...
#include "streetview_common.h"

namespace streetview {

class StreetviewParser {
 public:
  // Constants for filenames for calibration files.
  static const std::string kCameraFolder;
  static const std::string kPoseFolder;
  static const std::string kForwardCameraFolder;
  static const std::string kBackwardCameraFolder;
...
//  static const std::string kPosesFolder;
  ...
  StreetviewParser(const std::string& dataset_path, bool rectified);
  ...
  bool interpolatePoseAtTimestamp(uint64_t timestamp, streetview::Transformation* pose);
  ...
  bool getCameraCalibration(uint64_t cam_id, streetview::CameraCalibration* cam) const;
  ...
  streetview::Transformation T_camN_vel(int cam_number) const;
  streetview::Transformation T_camN_imu(int cam_number) const;

  // Basic accessors.
  streetview::Transformation T_cam0_vel() const;
  streetview::Transformation T_vel_imu() const;
 ...
 private:
  ...
  bool convertVectorToPose(const std::vector<double>& oxts, streetview::Transformation* pose);
  ...
  streetview::CameraCalibrationVector camera_calibrations_;

  // Transformation chain (cam-to-cam extrinsics stored above in cam calib
  // struct).
  streetview::Transformation T_cam0_vel_;
  streetview::Transformation T_vel_imu_;

  // Timestamp map from index to nanoseconds.
  // streetview has only one set of timestamps as all data is synchronized.
  ...
  streetview::Transformation T_initial_pose_;
  double mercator_scale_;
};
}

#endif  // STREETVIEW_TO_ROSBAG_STREETVIEW_PARSER_H_
