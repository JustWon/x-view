...
class StreetviewLiveNode {
 public:
  StreetviewLiveNode(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private,
  ...
  void publishTf(uint64_t timestamp_ns, const streetview::Transformation& imu_pose);
...
StreetviewLiveNode::StreetviewLiveNode(const ros::NodeHandle& nh,
...
: nh_(nh),
  ...
  for (size_t cam_id = 0; cam_id < parser_.getNumCameras(); ++cam_id) {
    image_pubs_.push_back(
        image_transport_.advertiseCamera(streetview::getCameraFrameId(cam_id), 1));
  }
}

void StreetviewLiveNode::startPublishing(double rate_hz) {
  ...
  publish_timer_ = nh_.createWallTimer(ros::WallDuration(publish_dt_sec),
                                       &StreetviewLiveNode::timerCallback, this);
}

void StreetviewLiveNode::timerCallback(const ros::WallTimerEvent& event) {
  streetview::Transformation tf_interpolated;
...
void StreetviewLiveNode::publishClock(uint64_t timestamp_ns) {
...
bool StreetviewLiveNode::publishEntry(uint64_t entry) {
  ros::Time timestamp_ros;
...
void StreetviewLiveNode::publishTf(uint64_t timestamp_ns,
                                const streetview::Transformation& imu_pose) {
  ros::Time timestamp_ros;
  streetview::timestampToRos(timestamp_ns, &timestamp_ros);
  streetview::Transformation T_imu_world = imu_pose;
  streetview::Transformation T_vel_imu = parser_.T_vel_imu();
  streetview::Transformation T_cam_imu;

  tf::Transform tf_imu_world, tf_cam_imu, tf_vel_imu;

  streetview::transformToTf(T_imu_world, &tf_imu_world);
  streetview::transformToTf(T_vel_imu.inverse(), &tf_vel_imu);
  ...
  for (size_t cam_id = 0; cam_id < parser_.getNumCameras(); ++cam_id) {
    ...
    streetview::transformToTf(T_cam_imu.inverse(), &tf_cam_imu);
    tf_broadcaster_.sendTransform(tf::StampedTransform(
        tf_cam_imu, timestamp_ros, imu_frame_id_, streetview::getCameraFrameId(cam_id)));
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "streetview_live");
  ...
  const std::string dataset_path =
      "/home/johnny/segnet/datasets/streetview";

  streetview::StreetviewLiveNode node(nh, nh_private, dataset_path);
