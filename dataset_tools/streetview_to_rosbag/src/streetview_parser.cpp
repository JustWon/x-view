...
#include "streetview_to_rosbag/depth_traits.h"
#include "streetview_to_rosbag/streetview_parser.h"

namespace streetview {

const std::string StreetviewParser::kDepthFolder = "Depth";
const std::string StreetviewParser::kSemanticsGTFolder = "Segmentation";
const std::string StreetviewParser::kRGBFolder = "RGB";
const std::string StreetviewParser::kForwardCameraFolder = "forward";
const std::string StreetviewParser::kBackwardCameraFolder = "backward";

StreetviewParser::StreetviewParser(const std::string& dataset_path, bool rectified)
: dataset_path_(dataset_path),
  rectified_(rectified),
  initial_pose_set_(false) {
  cam_paths_.push_back(kForwardCameraFolder);
  cam_paths_.push_back(kBackwardCameraFolder);
}

bool StreetviewParser::loadCalibration() {

  camera_calibrations_.resize(2);
  size_t cam_id = 0u;
  for (auto&& calibration : camera_calibrations_) {
    // Intrinsics.
    calibration.image_size << 480, 360;
...
bool StreetviewParser::parseVectorOfDoubles(const std::string& input,
...
void StreetviewParser::loadTimestampMaps() {

  std::string file_path = dataset_path_ + "/" + kRGBFolder + "/"
...
bool StreetviewParser::getCameraCalibration(uint64_t cam_id,
                                        streetview::CameraCalibration* cam) const {
...
bool StreetviewParser::convertDepthImageToDepthCloud(const sensor_msgs::Image& depth_image_msg,
...
bool StreetviewParser::getPoseAtEntry(uint64_t entry, uint64_t* timestamp,
                                  streetview::Transformation* pose) {
  // Read in both camera poses and spawn one in between.
  std::string filename_cam0 = dataset_path_ + "/waypoints_forward.txt";
  std::string filename_cam1 = dataset_path_ + "/waypoints_backward.txt";
...
bool StreetviewParser::getCameraPoseAtEntry(uint64_t entry, uint64_t id,
                                        streetview::Transformation* pose) {
  // Read in both camera poses and spawn one in between.
  std::string filename_cam;
  if (id == 0) {
    filename_cam = dataset_path_ + "/waypoints_forward.txt";
  } else if (id == 1) {
    filename_cam = dataset_path_ + "/waypoints_backward.txt";
  } else {
    return false;
  }
...
//        pose->getRotation() = streetview::Rotation(0.7071067811865476, 0, 0.7071067811865475, 0);
      } else {
...
uint64_t StreetviewParser::getPoseTimestampAtEntry(uint64_t entry) {
  if (timestamps_ns_.size() <= entry) {
    return 0;
  }
...
bool StreetviewParser::getImageAtEntry(uint64_t entry, uint64_t cam_id,
                                   uint64_t* timestamp, cv::Mat* image) {
  // Get the timestamp for this first.
  std::string filename = dataset_path_ + "/" + kRGBFolder + "/"
      + cam_paths_[cam_id] + "/" + getFilenameForEntry(entry) + ".jpeg";
...
bool StreetviewParser::getDepthImageAtEntry(uint64_t entry, uint64_t cam_id,
                                        uint64_t* timestamp, cv::Mat* depth_image) {
  // Get the timestamp for this first.

  std::string filename = dataset_path_ + "/" + kDepthFolder + "/"
      + cam_paths_[cam_id] + "/" + getFilenameForEntry(entry) + ".png";
...
bool StreetviewParser::getLabelImageAtEntry(uint64_t entry, uint64_t cam_id,
...
bool StreetviewParser::getLabelsAtEntry(uint64_t entry, uint64_t cam_id,
                                    uint64_t* timestamp, cv::Mat* label_image) {
  // Get the timestamp for this first.
  std::string filename = dataset_path_ + "/Segmentation/" + cam_paths_[cam_id]
      + "/" + getFilenameForEntry(entry) + ".png";
...
bool StreetviewParser::convertVectorToPose(const std::vector<double>& pose_7,
                                       streetview::Transformation* pose) {
  if (pose_7.size() < 7) {
    return false;
  }

  streetview::Transformation t_temp(
      Eigen::Vector3d(pose_7[0], pose_7[1], pose_7[2]),
      streetview::Rotation(pose_7[3], pose_7[4], pose_7[5], pose_7[6]));
  *pose = t_temp;
...
double StreetviewParser::latToScale(double lat) const {
  return cos(lat * M_PI / 180.0);
}

// From the MATLAB raw data dev kit.
void StreetviewParser::latlonToMercator(double lat, double lon, double scale,
...
std::string StreetviewParser::getFilenameForEntry(uint64_t entry) const {
  char buffer[20];
  sprintf(buffer, "%03d", entry);
  return std::string(buffer);
}

streetview::Transformation StreetviewParser::T_camN_vel(int cam_number) const {
  return camera_calibrations_[cam_number].T_cam0_cam.inverse() * T_cam0_vel_;
}

streetview::Transformation StreetviewParser::T_camN_imu(int cam_number) const {
  return T_camN_vel(cam_number) * T_vel_imu_;
}

streetview::Transformation StreetviewParser::T_cam0_vel() const { return T_cam0_vel_; }

streetview::Transformation StreetviewParser::T_vel_imu() const { return T_vel_imu_; }

bool StreetviewParser::interpolatePoseAtTimestamp(uint64_t timestamp,
                                              streetview::Transformation* pose) {
  ...
  streetview::Transformation transform_left, transform_right;
  ...
  *pose = streetview::interpolateTransformations(transform_left, transform_right, t);
  return true;
}

size_t StreetviewParser::getNumCameras() const {
