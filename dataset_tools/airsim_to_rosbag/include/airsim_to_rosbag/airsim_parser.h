#ifndef AIRSIM_TO_ROSBAG_AIRSIM_PARSER_H_
#define AIRSIM_TO_ROSBAG_AIRSIM_PARSER_H_
...
class AirsimParser {
 public:
  ...
  static const std::string kDownwardCameraFolder;
...
//  static const std::string kPosesFolder;
  ...
  AirsimParser(const std::string& dataset_path, bool rectified);
  ...
  bool interpolatePoseAtTimestamp(uint64_t timestamp, airsim::Transformation* pose);
  ...
  bool getCameraCalibration(uint64_t cam_id, airsim::CameraCalibration* cam) const;
  ...
  airsim::Transformation T_camN_vel(int cam_number) const;
  airsim::Transformation T_camN_imu(int cam_number) const;

  // Basic accessors.
  airsim::Transformation T_cam0_vel() const;
  airsim::Transformation T_vel_imu() const;
 ...
 private:
  ...
  bool convertVectorToPose(const std::vector<double>& oxts, airsim::Transformation* pose);
  ...
  airsim::CameraCalibrationVector camera_calibrations_;

  // Transformation chain (cam-to-cam extrinsics stored above in cam calib
  // struct).
  airsim::Transformation T_cam0_vel_;
  airsim::Transformation T_vel_imu_;

  // Timestamp map from index to nanoseconds.
  // Airsim has only one set of timestamps as all data is synchronized.
  ...
  airsim::Transformation T_initial_pose_;
  double mercator_scale_;
};
}

#endif  // AIRSIM_TO_ROSBAG_AIRSIM_PARSER_H_
