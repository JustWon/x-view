...
class AirsimLiveNode {
 public:
  AirsimLiveNode(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private,
  ...
  void publishTf(uint64_t timestamp_ns, const airsim::Transformation& imu_pose);
...
AirsimLiveNode::AirsimLiveNode(const ros::NodeHandle& nh,
...
: nh_(nh),
  ...
  for (size_t cam_id = 0; cam_id < parser_.getNumCameras(); ++cam_id) {
    image_pubs_.push_back(
        image_transport_.advertiseCamera(airsim::getCameraFrameId(cam_id), 1));
  }
}

void AirsimLiveNode::startPublishing(double rate_hz) {
  ...
  publish_timer_ = nh_.createWallTimer(ros::WallDuration(publish_dt_sec),
                                       &AirsimLiveNode::timerCallback, this);
}

void AirsimLiveNode::timerCallback(const ros::WallTimerEvent& event) {
  airsim::Transformation tf_interpolated;
...
void AirsimLiveNode::publishClock(uint64_t timestamp_ns) {
...
bool AirsimLiveNode::publishEntry(uint64_t entry) {
  ros::Time timestamp_ros;
...
void AirsimLiveNode::publishTf(uint64_t timestamp_ns,
                                const airsim::Transformation& imu_pose) {
  ros::Time timestamp_ros;
  airsim::timestampToRos(timestamp_ns, &timestamp_ros);
  airsim::Transformation T_imu_world = imu_pose;
  airsim::Transformation T_vel_imu = parser_.T_vel_imu();
  airsim::Transformation T_cam_imu;

  tf::Transform tf_imu_world, tf_cam_imu, tf_vel_imu;

  airsim::transformToTf(T_imu_world, &tf_imu_world);
  airsim::transformToTf(T_vel_imu.inverse(), &tf_vel_imu);
  ...
  for (size_t cam_id = 0; cam_id < parser_.getNumCameras(); ++cam_id) {
    ...
    airsim::transformToTf(T_cam_imu.inverse(), &tf_cam_imu);
    tf_broadcaster_.sendTransform(tf::StampedTransform(
        tf_cam_imu, timestamp_ros, imu_frame_id_, airsim::getCameraFrameId(cam_id)));
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "airsim_live");
  ...
  airsim::AirsimLiveNode node(nh, nh_private, dataset_path);
