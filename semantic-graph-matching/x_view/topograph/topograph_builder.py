...
class GraphEdge(object):
    ...
    def __init__(self, node1: AbstractGraphNode, node2: AbstractGraphNode, stiffness=1.0, rest_length=1.0,
                 edge_color='#125E87'):
        """Simply constructs an edge in the graph structure linking node1 and node2 by using their ID
        """
...
def generate_topograph(graph_node_list, graph_edge_list, name="TopoGraph"):
    ...
    for graph_node in graph_node_list:
        G.add_node(n=graph_node.node, pos=graph_node.pos, label=graph_node.label, node_color=graph_node.node_color,
                   landmarks=graph_node.landmarks)
