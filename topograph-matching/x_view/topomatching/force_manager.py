...
from x_view.restrictedClasses import restrictedSimilarity


class AbstractForceManager(object):
    ...
    def compute_inner_forces(self):
        raise NotImplementedError(
            "compute_inner_forces must be implemented in every subclass of '{}'".format(self.__class__.__name__))

    def compute_outer_forces(self):
        raise NotImplementedError(
            "compute_outer_forces must be implemented in every subclass of '{}'".format(self.__class__.__name__))
...
class WordnetForceManager(AbstractForceManager):
    def __init__(self, topograph_list, node_list, dict_list, inner_force=Hooke(k=10), outer_force=Hooke(k=20), dim=3):
        super(WordnetForceManager, self).__init__(topograph_list=topograph_list, node_list=node_list,
                                                  dict_list=dict_list, inner_force=inner_force, outer_force=outer_force,
                                                  dim=dim)
...
class RestrictedForceManager(AbstractForceManager):
    def __init__(self, topograph_list, node_list, dict_list, inner_force=Hooke(k=10), outer_force=Hooke(k=20), dim=3):
        super(RestrictedForceManager, self).__init__(topograph_list=topograph_list, node_list=node_list,
                                                     dict_list=dict_list, inner_force=inner_force,
                                                     outer_force=outer_force, dim=dim)
    ...
    def compute_outer_forces(self):
        ...
        for ((topo1_idx, (topo1, dicts1, nodes1)), (topo2_idx, (topo2, dicts2, nodes2))) in \
            ...
            for idx1, (node1, dict1) in enumerate(zip(nodes1, dicts1)):
                for idx2, (node2, dict2) in enumerate(zip(nodes2, dicts2)):
                    # compute semantic similarity
                    sim12 = restrictedSimilarity(node1.node.value, node2.node.value)
                    sim12 = 0 if sim12 < 0.3 else sim12
                    pos1, pos2 = dict1['pos'], dict2['pos']
