...
def get_restricted_topograph1(dim=3, num_elements=5):
    ...
    edges = generate_edges(nodes=graph_nodes, positions=positions, fill_factor=0.5, stretch_variation=0.1)
...
def get_restricted_topograph2(dim=3, num_elements=5):
    ...
    nodes = [NodeEntity(sem) for sem in semantic]
    colors = ["b" for _ in semantic]
    positions = [3 * np.ones(dim) + 3.5 * np.random.rand(dim) for _ in semantic]
    plot_positions = [pos[0:2] for pos in positions]
    ...
    edges = generate_edges(nodes=graph_nodes, positions=positions, fill_factor=0.4, stretch_variation=0.1)
