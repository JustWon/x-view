...
from ...restrictedClasses import semantic_concepts, restrictedSimilarity
...
class RestrictedGraphNode(AbstractGraphNode):
    def __init__(self, node, pos: np.array, label="restricted_node", lemma="", plot_pos=None, node_color='42C2ED'):
        super(RestrictedGraphNode, self).__init__(node=node, pos=pos, label=label, lemma=lemma, plot_pos=plot_pos,
                                                  node_color=node_color)
        if node.value not in semantic_concepts:
            raise ValueError(
                "'node.value' param passed as argumnent to constructor of '{}' must be contained in 'semantic_concepts'".format(
                    self.__class__.__name__))

    def similarity(self, other):
        if not isinstance(other, RestrictedGraphNode):
            return 0
        return restrictedSimilarity(self.node.value, other.node.value)
