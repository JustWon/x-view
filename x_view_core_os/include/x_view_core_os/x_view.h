...
#include <x_view_core_os/matchers/abstract_matcher.h>
...
class XView {
 ...
 public:
  ...
  void processFrameData_2(const FrameData& frame_data);

  void computeGraph(const int graph_size, Graph* out_graph);
  ...
  */
   ...
   */
  void writeGraphToFile(const std::string& prefix) const;
...
//  std::vector<SemanticGraphPtr> graph_landmarks_db_;
