...
void XViewBagReader::iterateBagFromTo(const CAMERA camera_type,
  ...
  for (int i = from; step * i < step * to; ) {
    if(!pause.isPaused()) {
      ...
      x_view_->processFrameData_2(frame_data);
      x_view_->writeGraphToFile("merged_");

      x_view_->computeGraph(i - from + 1, &merged_graph);
      graph_publisher_.clean();
      graph_publisher_.publish(merged_graph, ros::Time());
...
bool XViewBagReader::generateQueryGraph(const CAMERA camera_type,
  ...
  for(int i = start_frame; i < start_frame + steps; ++i) {
    ...
    local_x_view.processFrameData_2(frame_data);
    local_x_view.computeGraph(i - start_frame + 1, query_graph);
  ...
  local_x_view.writeGraphToFile(
      "localized_" + std::to_string(start_frame) + "_");
